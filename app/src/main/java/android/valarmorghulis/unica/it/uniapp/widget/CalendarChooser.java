package android.valarmorghulis.unica.it.uniapp.widget;
...
public class CalendarChooser extends FrameLayout {
    ...
    private static List<DummyContent.DummyItem> ITEMS = DummyContent.ITEMS;
    private static DummyContent.DummyItem mEvent;
    ...
    public static class DayTextView extends TextView {
        ...
        /**
         * The constants for the stat about an exam on the calendar
         */
        private static final int[] STATE_EXAM_1 = {R.attr.exam_1_to_show};
        private static final int[] STATE_EXAM_2 = {R.attr.exam_2_to_show};
        private static final int[] STATE_EXAM_3 = {R.attr.exam_3_to_show};
        ...
        /*
         * If true the item is related to an exam of the 1st year
         */
        private boolean mFirstYear = false;

        /*
         * If true the item is related to an exam of the 1st year
        */
        private boolean mSecondYear = false;

        /*
         * If true the item is related to an exam of the 1st year
        */
        private boolean mThirdYear = false;
         ...
         * Set the selectedDay state
         *
         * @param firstYear If true this is the selected day
         */
        public void setFirstYear(final boolean firstYear) {
         ...
         * Set the selectedDay state
         *
         * @param secondYear If true this is the selected day
         */
        public void setSecondYear(final boolean secondYear) {
            this.mSecondYear = secondYear;
            // In this case the states are mutual exclusive
            if (secondYear) {
                mToday = false;
                mSelectedDay = false;
         ...
         * Set the selectedDay state
         *
         * @param thirdYear If true this is the selected day
         */
        public void setThirdYear(final boolean thirdYear) {
            this.mThirdYear = thirdYear;
            // In this case the states are mutual exclusive
            if (thirdYear) {
                mToday = false;
                mSelectedDay = false;
        ...
        protected int[] onCreateDrawableState(int extraSpace) {
            ...
            if (mFirstYear){
                mergeDrawableStates(drawableState, STATE_EXAM_1);
            }
            if (mSecondYear){
                mergeDrawableStates(drawableState, STATE_EXAM_2);
            }
            if (mThirdYear){
                mergeDrawableStates(drawableState, STATE_EXAM_3);
            }
            ...
            if(DummyContent.isPresent(mCurrentDate.get(Calendar.DAY_OF_YEAR), mCurrentDate.get(Calendar.YEAR))){
                Toast.makeText(
                        ...
                        "Esame di " + ITEMS.get(DummyContent.selected).completeNameExam + " " + DummyContent.isPassed(ITEMS.get(DummyContent.selected)) +
                                " in data " + ITEMS.get(DummyContent.selected).getDateString(), Toast.LENGTH_SHORT)
                        .show();
            }
            // We update the UI
            updateUI();
    ...
    public void updateUI() {
        ...
        for (int week = 0; week < maxWeekNumber; week++) {
            ...
            for (int i = 0; i < DAYS_IN_A_WEEK; i++) {
                ...
                //System.out.println("Giorno: " + currentDayInYear + "/"+currentMonthYear+"/"+currentYear);
                //System.out.println("Giorno basi di dati: "+ illtry.));
                // We check if the current day is the selected day
                // System.out.println("i = " + i + ", ITEM = " + ITEMS.get(0).getDateExam().get(Calendar.DAY_OF_YEAR) + ", CDIY = " + currentDayInYear);
